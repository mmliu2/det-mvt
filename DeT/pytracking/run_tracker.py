...
def run_tracker(tracker_name, tracker_param, run_id=None, dataset_name='cdtb', dtype='rgb', sequence=None, debug=0, threads=0,
                visdom_info=None, save_features=''):
    ...
    run_dataset(dataset, trackers, debug, threads, visdom_info=visdom_info, run_id=run_id, save_features=save_features) # Song add run_id for box_file saving
...
def main():
    ...
    parser.add_argument('--save_features', type=str, default='', help='Whether to save features as .npy files.')
    ...
    if args.save_features != '':
        assert args.threads == 0 # feature saving is broken for multithreading

...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None, run_id=None, save_features=''):
    ...
    if debug:
        output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info, save_features=save_features)
    else:
        try:
            output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info, save_features=save_features)
...
def run_dataset(dataset, trackers, debug=False, threads=0, visdom_info=None, run_id=None, save_features=''):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug, visdom_info=visdom_info, run_id=run_id, save_features=save_features)
    elif mode == 'parallel':

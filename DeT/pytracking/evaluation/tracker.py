...
class Tracker:
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None, save_features=''):
        ...
        output = self._track_sequence(tracker, seq, init_info, save_features=save_features)
        return output

    def _track_sequence(self, tracker, seq, init_info, save_features=''):
        # Define outputs
        ...
        # for frame_num, frame_path in enumerate(zip(seq.frames[1:], seq.depth_frames[1:]), start=1):
            ...
            if save_features != '':
                out = tracker.track(image, info, save_features=os.path.join(save_features, seq.name))
            else:
                out = tracker.track(image, info, save_features=save_features)

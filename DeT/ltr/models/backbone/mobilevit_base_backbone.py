...
class BaseEncoder(nn.Module):
    ...
    def _forward_MobileViT_layer(self, layer: nn.Module, x: Tensor):
    ...
    # def _add_output_and_check(self, name, xz, outputs, output_layers): # new
    def _add_output_and_check(self, name, x, outputs, output_layers): # new
        # x, z = xz
        ...
        if name+'_A' in output_layers:
            outputs[name+'_A'] = x
        if name+'_B' in output_layers:
            outputs[name+'_B'] = x

        # return len(output_layers) == len(outputs[0])
        return len(output_layers) == len(outputs)

    # def forward_features(self, x, z, output_layers=['layer4'], train=True):
    def forward_features(self, x, output_layers=['layer4'], train=True):
        ...
        # if self._add_output_and_check('layer2', (x, z), outputs, output_layers):
        ...
        x = self._forward_MobileViT_layer(self.layer_3, x)

        # if self._add_output_and_check('layer3', (x, z), outputs, output_layers):
        ...
        x = self._forward_MobileViT_layer(self.layer_4, x)

        # if self._add_output_and_check('layer4', (x, z), outputs, output_layers):
        if self._add_output_and_check('layer4', (x), outputs, output_layers):
            return outputs

        assert False

    # def forward(self, x: Tensor, z: Tensor, output_layers=['layer4'], train=True):
    def forward(self, x: Tensor, output_layers=['layer4'], train=True):
        ...
        Returns:
            ...
            attn : None
        """
        # return self.forward_features(x, z, output_layers, train=train)
        return self.forward_features(x, output_layers, train=train)

...
def contrastive_loss(student_logits, teacher_logits, temperature=0.1):
    ...
    s = student_logits.permute(0, 2, 3, 1).reshape(-1, D)  # [B*H*W, T]
    t = teacher_logits.permute(0, 2, 3, 1).reshape(-1, D)  # [B*H*W, T]
    
    # Normalize
    s = F.normalize(s, dim=-1)
    t = F.normalize(t, dim=-1)
    
    # Similarity matrix: dot product / temperature
    sim = torch.matmul(s, t.T) / temperature  # [B*H*W, B*H*W]
...
class DiMPActorFeats(BaseActor):
    ...
    def __call__(self, data):
        ...
        loss_target_kl = self.loss_weight['test_clf'] * kl_loss_test

        # Loss for the initial filter iteration
        loss_test_init_kl = 0
        if 'test_init_clf' in self.loss_weight.keys():
            loss_test_init_kl = self.loss_weight['test_init_clf'] * kl_losses_test[0]
        ...
        if 'test_iter_clf' in self.loss_weight.keys():
            test_iter_weights = self.loss_weight['test_iter_clf']
            if isinstance(test_iter_weights, list):
                loss_test_iter_kl = sum([a*b for a, b in zip(test_iter_weights, kl_losses_test[1:-1])])
            else:
                loss_test_iter_kl = (test_iter_weights / (len(kl_losses_test) - 2)) * sum(kl_losses_test[1:-1])
        ...
        loss_target_contrastive = self.loss_weight['test_clf'] * contrastive_loss_test

        # Loss for the initial filter iteration
        loss_test_init_contrastive = 0
        if 'test_init_clf' in self.loss_weight.keys():
            loss_test_init_contrastive = self.loss_weight['test_init_clf'] * contrastive_losses_test[0]

        # Loss for the intermediate filter iterations
        loss_test_iter_contrastive = 0
        if 'test_iter_clf' in self.loss_weight.keys():
            test_iter_weights = self.loss_weight['test_iter_clf']
            if isinstance(test_iter_weights, list):
                loss_test_iter_contrastive = sum([a*b for a, b in zip(test_iter_weights, contrastive_losses_test[1:-1])])
            else:

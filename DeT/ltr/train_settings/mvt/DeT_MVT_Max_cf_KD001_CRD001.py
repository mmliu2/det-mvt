...
from ltr.dataset import Lasot, Got10k, TrackingNet, MSCOCOSeq, MSCOCOSeq_depth, Lasot_depth, CDTB, DepthTrack, DepthTrackFeats
from ltr.data import processing, sampler, LTRLoader
from ltr.models.tracking import mvt
...
def run(settings):
    ...
    settings.num_samples_train = 1000
    settings.num_samples_val = 1000
    settings.num_epochs = 25
    settings.backbone_pretrained_path = settings.env.pretrained_networks + '/mobilevit_s.pt'
    ...
    depthtrack_train = DepthTrackFeats(root=settings.env.depthtrack_dir, split='train', dtype=input_dtype)
    ...
    depthtrack_val = DepthTrackFeats(root=settings.env.depthtrack_dir, split='val', dtype=input_dtype)
    ...
    dataset_train = sampler.DiMPSamplerFeats([depthtrack_train], [1],
                                        samples_per_epoch=settings.num_samples_train, max_gap=30, num_test_frames=3, num_train_frames=3,
                                        processing=data_processing_train)
    ...
    dataset_val = sampler.DiMPSamplerFeats([depthtrack_val], [1], samples_per_epoch=settings.num_samples_val, max_gap=30,
                                      num_test_frames=3, num_train_frames=3,
                                      processing=data_processing_val)
    ...
    net = mvt.mvt_DeT(filter_size=settings.target_filter_sz, backbone_pretrained=settings.backbone_pretrained_path, optim_iter=5,
    ...
    actor = actors.DiMPActorFeats(net=net, objective=objective, loss_weight=loss_weight, kl_class_feats_weight=12.5, contrastive_class_feats_weight=0.125)

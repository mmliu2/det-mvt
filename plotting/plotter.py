...
df = pd.read_csv(data_path + 'mvt_train.txt', header=None)
mvt_train_loss = df.iloc[:, 1].tolist()
mvt_train_f1 = df.iloc[:, 4].tolist()

df = pd.read_csv(data_path + 'mvt_test.txt', header=None)
mvt_test_loss = df.iloc[:, 1].tolist()
mvt_test_f1 = df.iloc[:, 4].tolist()
...
axes[2].set_title("Depth-Adapted MVT Loss", fontsize=font_size)
...
axes[3].set_title("Depth-Adapted MVT F1-Score", fontsize=font_size)
...
plt.tight_layout()
plt.savefig(data_path + 'figs/temp2.png')
plt.show()
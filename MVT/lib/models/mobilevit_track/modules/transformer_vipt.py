...
class TransformerEncoderPrompt(BaseModule):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.prompt_block_dte = Prompt_block(inplanes=embed_dim, hide_channel=8)
    ...
    def forward(
        ...
        **kwargs
        ...
        # vipt
        patches_x = x[:, 0:info_dicts['x']['total_patches'], :]
        patches_z = x[:, info_dicts['x']['total_patches']:, :]
        
        fm_x = folding(patches=patches_x, info_dict=info_dicts['x'])
        fm_z = folding(patches=patches_z, info_dict=info_dicts['z'])

        patches_x_prompt = prompt[:, 0:info_dicts['x']['total_patches'], :]
        patches_z_prompt = prompt[:, info_dicts['x']['total_patches']:, :]
        
        fm_x_prompt = folding(patches=patches_x_prompt, info_dict=info_dicts['x_dte'])
        fm_z_prompt = folding(patches=patches_z_prompt, info_dict=info_dicts['z_dte'])
        
        x_cat = torch.cat((fm_x, fm_x_prompt), 1)
        z_cat = torch.cat((fm_z, fm_z_prompt), 1)

        x_cat = self.prompt_block_dte(x_cat)
        z_cat = self.prompt_block_dte(z_cat)

        patches_x_updated, info_dict_x = unfolding(x_cat)
        patches_z_updated, info_dict_z = unfolding(z_cat)
        info_dicts['x'] = info_dicts['x_dte'] = info_dict_x
        info_dicts['z'] = info_dicts['z_dte'] = info_dict_z

        concatenated_patches = torch.cat((patches_x_updated, patches_z_updated), 1)
        concatenated_patches_prompt = concatenated_patches.clone()

        x = concatenated_patches
        ...
        return x, concatenated_patches_prompt, info_dicts

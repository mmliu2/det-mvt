...
from lib.utils.box_ops import box_cxcywh_to_xyxy, box_xywh_to_xyxy, box_pixelwise_metrics
import torch
...
class MobileViTViPTTrackActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        args:
            ...
            template_images_rgb: (N_t, batch, 3, H, W)
            search_images_rgb: (N_s, batch, 3, H, W)
            template_images_d: (N_t, batch, 3, H, W)
            search_images_d: (N_s, batch, 3, H, W)
        returns:
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        try:
            ...
            if self.settings.use_gpu:
                ...
                giou_loss, iou = torch.tensor(0.0).cpu(), torch.tensor(0.0).cpu()
        ...
        # DEPTHTRACK METRICS
        tp, fp, fn = self.objective['pixelwise'](pred_boxes_vec, gt_boxes_vec)  # (BN,4) (BN,4)
        ...
        if return_status:
            ...
            stats = {"depth/loss": loss.item(),
                      "depth/f1": f1.item(),
                      "depth/precision": pr.item(),
                      "depth/recall": re.item(),
